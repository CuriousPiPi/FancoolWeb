 ...
 *
 * 职责:
 * 1. 管理曲线 key 与颜色索引 (color_index) 之间的映射。
 * 2. 持久化颜色映射到 localStorage，确保刷新后颜色不变。
 * 3. 根据当前亮/暗主题，动态生成并提供调色板。
 * 4. 提供为单条或批量曲线分配唯一颜色索引的健壮方法。
 * 5. 提供释放不再使用的颜色索引的机制。
 * 6. 暴露简单的 API 供上层调用 (getColor, assignIndices, releaseIndex, etc.)。
 */
